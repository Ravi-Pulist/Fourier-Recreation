# -*- coding: utf-8 -*-
"""Presentation file for visualizing epicycle while drawing fourier transformed
 images via closed loop function. Presentation libraries such as PIL, Matplot
 -lib pylab, numpy, math, scipy are used."""
...
def create_close_loop(image_name, level=[200]):
    ...
    #read image to array, then image border with countour
    im = np.array(Image.open("picture.jpg").convert('L'))
    contour_plot = ax[0].contour(im, levels=level, colors='black', origin = 'image')
    print(contour_plot)
    #print(im)
    
    #Get Countor Path and create lookup-table
...
#            #circle.set_data(x, y)
#            #pos = new_pos            
#        for j, circle in zip(idx, circles):
#            new_pos = pos + c[j]
#            #rad.set_data([pos[0], new_pos[0], [pos[1], new_pos[1]]])
#            theta = np.linspace(0, tau, 50)
#            x, y = r[j] * np.cos(theta) + pos[0], r[j] * np.sin(theta) + pos[1]
...
#Step 2: Parameterize Image as a closed loop fuction
time_table, x_table, y_table = create_close_loop('picture.jpg')
...
#anim.save('picture.html', writer=writer, dpi=150)    
